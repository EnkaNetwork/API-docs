...
คุณสามารถใช้ไลบรารี่ที่ถูกสร้างขึ้นโดยคนอื่น (API Wrapper) หรือใช้ API โดยตรงได้ ซึ่งจะได้เป็นข้อมูลในรูปแบบที่ไม่ได้ซับซ้อนอะไร ดังนั้นมันก็ไม่ใช่เรื่องยากที่จะสร้างสมการมาคำนวณให้ได้เป็นข้อมูลสมบูรณ์ด้วยตัวคุณเอง สิ่งที่ยากที่สุดคือการนำเอา asset เกมมาใช้ร่วมกับข้อมูลที่ได้จาก API นี้

โปรดดูที่ [Wrappers](#wrappers) สำหรับไลบรารี่ทั้งหมดของแต่ละภาษาที่รองรับ

## ก่อนที่คุณจะส่งข้อมูล

กฏของการใช้งาน API มีอยู่ 4 ข้อ:

1. โปรดอย่าใช้ระบบเดา UID หรือพยายามที่จะดึงข้อมูลเพื่อเก็บไว้ใช้จำนวนมหาศาล เนื่องจากมี UID ที่เป็นไปได้ทั้งจำนวนมาก และ API นี้ไม่มีความสามารถที่จะกระทำข้างต้นได้

2. โปรดกำหนด `User-Agent` ไว้ที่ Header ของ API request ตามชื่อสิ่งที่คุณกำลังทำ เพื่อให้สามารถติดตามและช่วยแก้ไขปัญหาได้รวดเร็ว

3. เอนด์พอยต์ UID ของ API มีการจำกัดอัตราตามการใช้งาน - ถ้าหากโค้ดคุณ request ข้อมูลถี่เกินไป API จะตอบสนองช้าลง และถ้ายังคงดึงข้อมูลต่อไป จะทำให้ API ส่ง `429 Too Many Requests` กลับมาซึ่งแปลว่าคุณถึงขีดจำกัดแล้ว เพราะฉะนั้นหมายความว่าคุณต้องลดความเร็วการ request ลงหรือติดต่อเราเพื่อให้เพิ่ม ratelimits ให้คุณ แต่ในกรณีส่วนใหญ่นั้นไม่มีความจำเป็นและเกิดจาก code ที่ส่งมานั้นไม่มีประสิทธิภาพมากพอ

4. ในทุก UID request นั้นจะได้รับคีย์ `ttl` - field นี้คือ "จำนวนวินาทีที่ API จะเก็บแคชข้อมูลเก่าไว้ ก่อนจะดึงข้อมูลใหม่" โดยถ้าคุณส่ง request ซ้ำหลายรอบก่อน `ttl` จะหมด API จะส่งข้อมูลเก่าให้คุณแทน แต่แน้นว่าในกรณีนี้คุณจะยังคงใช้ ratelimit ถึงแม้ว่าจะแคชไว้ โปรดลองแคชข้อมูลนี้ในโค้ดของคุณเองตาม `ttl` หรือ พยายามอย่าดึงข้อ UID เดิมจนกว่า `ttl` จะหมด เราแนะนำให้ใช้ Redis สำหรับสิ่งนี้

หากคุณประสบปัญหาในการทำงานร่วมกับกับข้อมูล API ท่านสามารถเข้ามาทักถามได้ใน [เซิร์ฟเวอร์ Discord](https://discord.gg/PcSZr5sbn3)
